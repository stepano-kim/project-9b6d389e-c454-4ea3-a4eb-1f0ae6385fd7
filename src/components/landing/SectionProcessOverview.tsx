...
const steps = [{
  ...
  bullets: ["전기요금 전체가 아니라 설비·공간·운영 패턴 기준으로 에너지 사용 구조를 분석", "지금 건물에서 효과가 나올 수 있는 영역부터 선별"]
}, {
  step: "STEP 2",
  icon: Settings,
  title: "구축",
  subtitle: "최적의 설계로 에너지 최적화를 위한 구축을 진행합니다",
  bullets: ["진단 결과를 바탕으로 불필요한 범위를 제외한 최적의 설계", "필요한 경우에만 에너지 최적화를 위한 설비 교체 및 구축 진행"]
}, {
  step: "STEP 3",
  icon: BarChart3,
  title: "운영",
  subtitle: "정밀한 모니터링과 최적화 제어로 에너지 최적화를 실현합니다",
  bullets: ["구축 이후에는 실시간 모니터링과 데이터 기반 운영", "상황에 맞는 제어를 통해 지속적으로 에너지 사용을 최적화"]
}];
export function SectionProcessOverview() {
  return <section className="section-padding section-subtle">
      <div className="container-tight">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {steps.map((item, index) => <motion.div key={item.step} initial={{
          opacity: 0,
          y: 30
        }} whileInView={{
          opacity: 1,
          ...
          delay: index * 0.1
              ...
              <ul className="space-y-2">
                {item.bullets.map((bullet, i) => <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
